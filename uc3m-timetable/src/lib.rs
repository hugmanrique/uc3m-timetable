...
/// A [`Result`](std::result::Result) alias where the
...
impl TimetableId {
    ...
    pub fn url(&self) -> Url {
        let url = format!(
            "https://aplicaciones.uc3m.es/horarios-web/publicacion/{}/porCentroPlanCursoGrupo.tt",
            self.year
        );
        let params = [
...
#[cfg(test)]
mod tests {
    use crate::{TimetableId, UC3M_TIMEZONE};
